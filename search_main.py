...
GET_LIMIT = 50      # サムネイルは50件！
LP_LIMIT = 5        # 詳細分析は5件！
SAVE_DIR = "seo_lp_stock"        
...
def create_review_report(all_data, date_str):
    ...
    for item in all_data:
        ...
        html += f"""<div class="item-box"><div class="cat-name">🔍 {item['keyword']} 【{item['rank']}位】</div><div class="item-title">{item['title']}</div><div style="margin:10px 0;"><div style="font-size:12px; font-weight:bold; margin-bottom:5px; color:#666;">抽出キーワード</div>{keywords_html}</div><div class="reason-box">💡 <b>AI分析メモ:</b><br>{item['reason']}</div>{f'<div style="text-align:right;"><a href="{item["review_url"]}" target="_blank" class="link-btn">実際のレビューを見る</a></div>' if item['review_url'] else ''}</div>"""
    html += "</div></body></html>"
...
async def run_fixed():
    ...
    async with async_playwright() as p:
        ...
        for keyword in TARGET_KEYWORDS:
            log(f"\n🔍 キーワード【{keyword}】の検索順位を取得中...")
            search_url = f"https://search.rakuten.co.jp/search/mall/{urllib.parse.quote(keyword)}/"
            context_pc = await browser.new_context(viewport={'width': 1280, 'height': 800}, user_agent=pc_ua)
            page_pc = await context_pc.new_page()
            target_items = []
            try:
                await page_pc.goto(search_url, timeout=60000, wait_until="domcontentloaded")
                for _ in range(5):
                    await page_pc.evaluate("window.scrollBy(0, 2000)")
                    await page_pc.wait_for_timeout(500)
                extracted_items = await page_pc.evaluate(f"""
                    () => {{
                        const items = [];
                        const links = document.querySelectorAll("a[href*='item.rakuten.co.jp']");
                        for(let link of links) {{
                            if(items.length >= {GET_LIMIT}) break;
            ...
            for i, item in enumerate(target_items):
                ...
                res = {"keyword": keyword, "rank": rank, "url": item['url'], "thumb_url": item['thumb'], "is_full": is_full, "title": "", "catch_copy": "", "review_url": "", "review_summary": "なし", "type": "－", "reason": "", "img": "", "color": "#ccc"}
                try:
                    if is_full:
                        ...
                        safe_kw = "".join(c for c in keyword if c.isalnum() or c in [' ', '　']).replace(' ', '_')
                        img_name = f"{today_str}_{safe_kw}_rank{rank}.jpg"
    ...
    if all_data_list:
        pd.DataFrame(all_data_list).to_csv(os.path.join(SAVE_DIR, f"rakuten_seo_list_{today_str}.csv"), index=False, encoding="utf-8-sig")
        create_review_report(all_data_list, today_str)

        html = f"""<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="referrer" content="no-referrer"><meta name="robots" content="noindex, nofollow"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>SEO検索LP分析 ({today_str})</title><style>body {{ font-family: "Helvetica Neue", Arial, sans-serif; background: #f0f2f5; padding: 20px; color: #333; }} h1 {{ text-align: center; margin-bottom: 20px; }} .nav-area {{ text-align: center; margin-bottom: 30px; }} .nav-btn {{ display: inline-block; margin: 5px; padding: 10px 20px; background: #fff; color: #007bff; text-decoration: none; border-radius: 20px; font-weight: bold; box-shadow: 0 2px 5px rgba(0,0,0,0.1); transition: 0.2s; }} .nav-btn:hover {{ transform: translateY(-2px); box-shadow: 0 5px 10px rgba(0,0,0,0.15); }} .thumb-matrix {{ display: grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 12px; background: white; padding: 20px; border-radius: 10px; margin-bottom: 30px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); }} .matrix-item {{ display: flex; flex-direction: column; align-items: center; text-decoration: none; color: #333; font-size: 12px; position: relative; transition: transform 0.2s; }} .matrix-item:hover {{ transform: scale(1.05); z-index: 10; }} .matrix-img {{ width: 100%; aspect-ratio: 1/1; object-fit: cover; border-radius: 8px; border: 1px solid #ddd; }} .matrix-ext {{ position: absolute; top: 0; right: 0; background: #333; color: white; font-size: 9px; padding: 2px 4px; opacity: 0.8; border-radius: 0 8px 0 4px; }} .matrix-rank {{ margin-top: 5px; font-size: 13px; font-weight: bold; background: #007bff; color: white; padding: 2px 8px; border-radius: 10px; }} .gallery {{ display: flex; flex-wrap: wrap; gap: 20px; justify-content: flex-start; }} .card {{ background: white; width: 320px; padding: 15px; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); display: flex; flex-direction: column; border-top: 4px solid #007bff; }} .rank-header {{ display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; }} .thumb-wrapper {{ height: 350px; overflow: hidden; border: 1px solid #eee; cursor: zoom-in; border-radius: 6px; }} .thumb {{ width: 100%; transition: opacity 0.3s; }} .thumb:hover {{ opacity: 0.8; }} .modal {{ display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.85); z-index: 999; backdrop-filter: blur(5px); }} .modal-content {{ position: relative; margin: 30px auto; width: 90%; max-width: 450px; height: 90vh; background: white; overflow-y: auto; border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.5); }} .close {{ position: sticky; top: 0; right: 0; background: #fff; padding: 15px; text-align: right; font-size: 24px; cursor: pointer; border-bottom: 1px solid #eee; z-index: 100; }}</style><script>function openModal(src){{document.getElementById('mImg').src=src;document.getElementById('modal').style.display='block';document.body.style.overflow='hidden';}} function closeModal(){{document.getElementById('modal').style.display='none';document.body.style.overflow='auto';}}</script></head><body><h1>🔍 SEO検索 LP分析 ({today_str})</h1><div class="nav-area"><a href="../index.html" class="nav-btn">🏠 ホームに戻る</a><a href="index.html" class="nav-btn">📂 過去の日付一覧に戻る</a></div><div id="modal" class="modal" onclick="closeModal()"><div class="modal-content" onclick="event.stopPropagation()"><div class="close" onclick="closeModal()">×</div><img id="mImg" style="width:100%;"></div></div>"""
        for kw in TARGET_KEYWORDS:
            items = [x for x in all_data_list if x['keyword'] == kw]
            if not items: continue
            html += f"<h2>🔍 {kw} (上位{len(items)}件)</h2><div class='thumb-matrix'>"
            for it in items:
                link = f"#{''.join(c for c in kw if c.isalnum() or c in [' ', '　']).replace(' ', '_')}_{it['rank']}" if it['is_full'] else it['url']
                target = "" if it['is_full'] else 'target="_blank"'
                ext = "<span class='matrix-ext'>楽天↗</span>" if not it['is_full'] else ""
                html += f"<a href='{link}' class='matrix-item' {target}><img src='{it['thumb_url'] or 'https://placehold.co/100'}' class='matrix-img'><span class='matrix-rank'>{it['rank']}位</span>{ext}</a>"
            html += "</div><div class='gallery'>"
            for it in items:
                if not it['is_full']: continue
                rid = "".join(c for c in kw if c.isalnum() or c in [' ', '　']).replace(' ', '_') + f"_{it['rank']}"
        ...
        idx = """<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="referrer" content="no-referrer"><meta name="robots" content="noindex"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>SEO LPアーカイブ</title><style>body { font-family: "Helvetica Neue", Arial, sans-serif; padding: 20px; background: #f4f7f6; color: #333; } .container { max-width: 600px; margin: 0 auto; } h1 { text-align: center; color: #007bff; margin-bottom: 30px; } .home-btn { display: block; width: fit-content; margin: 0 auto 30px; padding: 10px 20px; background: #fff; color: #333; text-decoration: none; border-radius: 20px; font-weight: bold; box-shadow: 0 2px 5px rgba(0,0,0,0.1); } .list-group { list-style: none; padding: 0; } .list-item { margin-bottom: 15px; } .report-link { display: block; padding: 20px; background: #fff; border-radius: 10px; text-decoration: none; color: #333; font-weight: bold; box-shadow: 0 2px 5px rgba(0,0,0,0.05); transition: 0.2s; border-left: 6px solid #007bff; } .report-link:hover { transform: translateY(-3px); box-shadow: 0 5px 15px rgba(0,0,0,0.1); } .date-text { font-size: 18px; } .arrow { float: right; color: #ccc; }</style></head><body><div class="container"><h1>🔍 SEO LPアーカイブ</h1><a href="../index.html" class="home-btn">🏠 ホームに戻る</a><ul class="list-group">"""
        for p in files:
            fname = os.path.basename(p)
